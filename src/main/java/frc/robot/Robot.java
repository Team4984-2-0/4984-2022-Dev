...
//  Compressor compressor = new Compressor(0, PneumaticsModuleType.CTREPCM);
  ...
  public boolean OnTarget(double distance) {

    ResetEncoders();
    navX.reset();
    double motorPower = .75;
    shouldContinue = true;
      while ((-rightDriveEncoder.getDistance())<=distance*.8 && shouldContinue == true) {
        double angle = navX.getAngle();
        DriveCurve(motorPower, angle* ProportionalScale);
        //needs wait command
        try {
          Thread.sleep(50);
      ...
      while((-rightDriveEncoder.getDistasnce())<=distance && shouldContinue == true) {
        double angle = navX.getAngle();
        DriveCurve(.4, angle* ProportionalScale);
        //needs wait command
        try {
          Thread.sleep(50);
        }
      ...
      System.out.println("right= "+ rightDriveEncoder.getDistance());
